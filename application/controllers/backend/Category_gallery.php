<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    function proses()
   {
            ...
            { 
               $data = array();
                    $this->load->model('m_login');
                        ...
                        $id_category_gallery = $this->uri->segment(4);
		            $data['single_category_gallery'] = $this->category_gallery_model->show_category_gallery_id($id_category_gallery);
		      $data['title'] = ' EDIT CATEGORY GALLERY';   
            $data['main'] = 'backend/Edit_category_gallery';
        $this->load->view('backend/Main_v', $data);
    }
    function update_category_gallery($id_category_gallery) {
        {
              if($this->session->userdata('isLogin') == FALSE)
    {
      redirect('backend/login/login_form');
        ...
        { 
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
			...
			$this->db->where('id_category_gallery', $id_category_gallery);
    ...
    {    $data = array();
        ...
        $config['per_category_gallery'] = "10";
        $config["uri_segment"] = 4;
        $choice = $config["total_rows"] / $config["per_category_gallery"];
        ...
        $data['category_gallery'] = $this->category_gallery_model->get_category_gallery_list($config["per_category_gallery"], $data['page']);           
  ...
  redirect('backend/category_gallery/cek');     
      ...
      function delete($id_category_gallery){
