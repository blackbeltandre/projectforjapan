<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class category_portfolio extends CI_Controller { 
		function __construct()
    	{
            ...
            $this->load->model(array('m_login','category_portfolio_model'));   
        $this->load->database();
	}
    function index()
	   {
		if($this->session->userdata('isLogin') == FALSE)
        {
            ...
            {    
		        ...
		        $data['title'] = ' INSERT CATEGORY PORTFOLIO';   
            $data['main'] = 'backend/category_portfolio';
    ...
    function proses()
   {
        ...
        if ($this->form_validation->run() == FALSE)
    {
		...
		$create  = $this->db->insert('category_portfolio',$data);
        if ($create) $this->session->set_flashdata('message', "<div class='alert alert-dismissable alert-info'><button type='button' class='close' data-dismiss='alert'>
             ...
             $this->session->set_flashdata('message', "<div class='alert alert-info alert-block'><button type='button' class='close' data-dismiss='alert'>
                ...
                redirect('backend/category_portfolio/cek'); 	
            }
        }
       
            function edit_category_portfolio()
	{
		  if($this->session->userdata('isLogin') == FALSE)
            {
                redirect('backend/login/login_form');
            ...
            { 
               $data = array();
                    $this->load->model('m_login');
                        ...
                        $id_category_portfolio = $this->uri->segment(4);
		            $data['single_category_portfolio'] = $this->category_portfolio_model->show_category_portfolio_id($id_category_portfolio);
		      $data['title'] = ' EDIT CATEGORY PORTFOLIO';   
            $data['main'] = 'backend/edit_category_portfolio';
        $this->load->view('backend/main_v', $data);
    }
    function update_category_portfolio($id_category_portfolio) {
        {
              if($this->session->userdata('isLogin') == FALSE)
    {
      redirect('backend/login/login_form');
        ...
        { 
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
			...
			$this->db->where('id_category_portfolio', $id_category_portfolio);
		$update = $this->db->update('category_portfolio',$data);
    ...
    {    $data = array();
        ...
        $config['base_url'] = site_url('backend/category_portfolio/cek');
        $config['total_rows'] = $this->db->count_all('category_portfolio');
        $config['per_category_portfolio'] = "10";
        $config["uri_segment"] = 4;
        $choice = $config["total_rows"] / $config["per_category_portfolio"];
        ...
        $data['category_portfolio'] = $this->category_portfolio_model->get_category_portfolio_list($config["per_category_portfolio"], $data['page']);           
        $data['pagination'] = $this->pagination->create_links();
        $data['title'] = ' LIST CATEGORY PORTFOLIO';   
        $data['main'] = 'backend/list_category_portfolio';
        $this->load->view('backend/main_v', $data);
    ...
    {    $data = array();
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
           ...
           $multiple = $this->category_portfolio_model->multiple_delete();
  ...
  redirect('backend/category_portfolio/cek');     
      ...
      function delete($id_category_portfolio){
       
        $this->db->where('id_category_portfolio', $id_category_portfolio);
        $delete =  $this->db->delete('category_portfolio');
