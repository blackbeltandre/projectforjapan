...
class Pages extends CI_Controller { 
		function __construct()
	{
        ...
        $this->load->model(array('m_login','pages_model'));   
        $this->load->database();
	}
...
function index()
    ...
    {    $data = array();
        ...
        $data['list_pages'] = $this->pages_model->show_list_pages()->result_array();
    		$data['title'] = ' INSERT PAGES';   
        $data['main'] = 'backend/Pages';
...
function proses()
   {
    ...
    {    $data = array();
        ...
        $this->form_validation->set_rules('title', 'Judul pages', 'required|min_length[3]|max_length[200]');
        ...
        if ($this->form_validation->run() == FALSE)
    {
		...
		$create = $this->db->insert('pages',$data);
    ...
    {    $data = array();
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
           ...
           $multiple = $this->pages_model->multiple_delete();
  ...
  redirect('backend/pages/cek');     
    ...
    {    $data = array();
        ...
        $data['page'] = $this->pages_model->show_id_page($id)->result_array();
		$data['single_pages'] = $this->pages_model->show_pages_id($id);
		$data['title'] = ' EDIT PAGES';   
        $data['main'] = 'backend/Edit_pages';
        $this->load->view('backend/Main_v', $data);
...
function update_pages($id) 
    ...
    {   
        ...
        $config['base_url'] = site_url('backend/pages/cek');
        ...
        $data['pages'] = $this->pages_model->get_pages_list($config["per_page"], $data['page']);           
        $data['pagination'] = $this->pagination->create_links();    
        $data['title'] = ' LIST PAGES';   
        $data['main'] = 'backend/List_pages';
        $this->load->view('backend/Main_v', $data);
      } 
      function delete($id){
        $this->db->where('id',$id);
        ...
        $this->db->delete('pages', array('foto' => $id));
        $this->db->where('id', $id);
